...
func InitAppConfig(dataFile string, ymlDefault string) {
	...
	// filePaths := path.Join("", "config.yaml.example")
	// _, erre := os.Stat(filePaths)
	// if erre != nil {
	// 	log.Printf("config file path %s not exist", filePaths)

	// 	panic(R.ReturnFailMsg("config file path " + filePaths + " not exist"))
	// }
